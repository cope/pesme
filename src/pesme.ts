...
const extract = (folder: string, wordfile: string): void => {
	const ext = path.extname(wordfile);
	if (ext !== '.docx') return;

	if (!fs.existsSync(path.join(folder, 'pesme'))) fs.mkdirSync(path.join(folder, 'pesme'));
	...
	extracted.then((doc: any) => {
		...
		for (const pesma of pesme) {
			...
			// console.log(folder, wordfile, 'allFiles.push', path.basename(newFile));
		}
	});
};
